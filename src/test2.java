...
public class test2{
    ...
    static boolean blabla(String[] entree, Jeu jeu){
        if((entree[0].equals("s") && jeu.getPlayer().getSideSize() !=0 && Integer.parseInt(entree[1]) <= jeu.getPlayer().getSideSize()) || (entree[0].equals("p") && jeu.accessible(jeu.getPlayer().getPyramid(),Integer.parseInt(entree[1]), Integer.parseInt(entree[2])))){
            return true;
        }else{
            return false;
        }
